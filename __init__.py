...
class FilmNoirSkill(OVOSCommonPlaybackSkill):
    ...
    def __init__(self):
        ...
        self.archive = IAArchivist("Film_Noir")
    ...
    def match_skill(self, phrase, media_type):
        score = 0
        if self.voc_match(phrase, "movie") or media_type == MediaType.BLACK_WHITE_MOVIE:
            score += 10
        if self.voc_match(phrase, "old"):
            score += 30
        if self.voc_match(phrase, "public_domain"):
            score += 15
        if self.voc_match(phrase, "noir"):
            score += 70
        return score
